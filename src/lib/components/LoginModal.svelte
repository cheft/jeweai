<script lang="ts">
	...
	let email = $state('');
	let password = $state('');
	let step = $state<'email' | 'password' | 'sent'>('email');
	...
	async function handleContinue() {
		...
		try {
			const { exists } = await client.auth.checkEmail({ email });
			if (exists) {
				step = 'password';
			} else {
				// Initialize registration
				await client.auth.registerInit({ email });
				step = 'sent';
			}
		} catch (e: any) {
		...
		>
			...
			<div class="relative z-10 p-8">
				<div class="mb-8 text-center">
					...
					{#if step === 'sent'}
						...
						<p class="text-sm text-gray-400">
							We've sent a temporary login link to <span class="text-white">{email}</span>. Click
				...
				{#if step !== 'sent'}
					<div class="space-y-4">
						<button
							class="flex w-full items-center justify-center gap-3 rounded-xl bg-white px-4 py-3 font-bold text-black transition-colors hover:bg-gray-100 disabled:opacity-50"
							disabled={loading}
						>
						...
						<form class="space-y-4" onsubmit={(e) => e.preventDefault()}>
							<div>
								...
								<input
									...
									class="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-white placeholder-gray-600 transition-all focus:border-seko-accent focus:ring-1 focus:ring-seko-accent focus:outline-none disabled:opacity-50"
							...
							<button
								...
								class="hover:bg-opacity-90 flex w-full items-center justify-center rounded-xl bg-seko-accent px-4 py-3 font-bold text-black shadow-[0_0_15px_rgba(163,230,53,0.2)] transition-colors disabled:opacity-50"
							>
								{#if loading}
								...
								{/if}
								{step === 'email' ? 'Continue with Email' : 'Sign in with Email'}
							</button>
							...
							{#if step === 'password'}
								<button
									type="button"
									onclick={() => (step = 'email')}
									class="w-full text-center text-xs text-gray-500 transition-colors hover:text-white"
								>
				...
				{:else}
					<div class="mt-4 flex flex-col items-center gap-6">
						<div class="rounded-full bg-seko-accent/10 p-4">
							<svg
								xmlns="http://www.w3.org/2000/svg"
								class="h-12 w-12 text-seko-accent"
								fill="none"
								viewBox="0 0 24 24"
								stroke="currentColor"
							>
						...
						<button
							onclick={() => (step = 'email')}
							class="text-sm text-seko-accent hover:underline"
						>
							Back to sign in
						</button>
					</div>
