...
>
	<div
		class="relative max-h-[90vh] w-full max-w-6xl overflow-y-auto rounded-2xl border border-white/10 bg-seko-bg bg-[radial-gradient(ellipse_at_top,var(--tw-gradient-stops))] from-seko-accent/5 via-seko-bg to-seko-bg p-6 shadow-2xl"
		transition:fade={{ duration: 200 }}
		onclick={(e) => e.stopPropagation()}
	>
		<!-- Close Button -->
		<button
			class="absolute right-4 top-4 z-10 rounded-full bg-black/50 p-2 text-gray-400 transition-colors hover:bg-black/70 hover:text-white"
			onclick={onClose}
			aria-label="Close"
		>
			<X class="h-5 w-5" />
		...
		{:else if asset}
			...
			<div class="grid grid-cols-1 gap-8 lg:grid-cols-3">
				<!-- Main Content (Video/Image Player) -->
				<div class="space-y-6 lg:col-span-2">
					...
					<div class="flex items-center justify-between">
						<div class="flex items-center gap-4">
							...
							<div>
								<h1 id="asset-detail-title" class="text-xl font-bold text-white">
									{asset.name}
								</h1>
						...
						<div class="flex gap-3">
							{#if asset.videoUrl || asset.originalImageUrl}
								<a
									href={asset.videoUrl || asset.originalImageUrl}
									download
					...
					<div class="rounded-2xl border border-white/5 bg-white/2 p-6">
						...
						<div class="space-y-3">
							...
							<div class="flex justify-between text-sm">
								...
								<span class="font-mono text-gray-300">{formatString} · {asset.type === 'video' ? 'MP4' : 'PNG'}</span>
							</div>
							<div class="flex justify-between text-sm">
